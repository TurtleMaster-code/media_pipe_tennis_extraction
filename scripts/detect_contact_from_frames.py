...
def detect_contact(frames):
    """Find frame where racket tip (index finger) is highest."""
    best_frame = None
    ...
    for frame_path in frames:
        ...
        index_tip = lm[mp_pose.PoseLandmark.RIGHT_INDEX].y  # racket tip proxy

        # Highest point = minimum y
        height = min(wrist, elbow, shoulder, index_tip)
...
output_base = r"C:\Users\arhan\OneDrive\Pictures\Documents\GitHub\Tennis_Pos_Extraction_Plus_Ai_Feedback\videos\right_side_views\contact_frames"

os.makedirs(output_base, exist_ok=True)

csv_path = os.path.join(output_base, "contact_frames.csv")

with open(csv_path, "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["serve_number", "contact_frame", "saved_image"])
    ...
    for i in range(1, 12):
        ...
        best_frame = detect_contact(frames)
        ...
        out_path = os.path.join(output_base, f"serve{i}_contact.png")
        cv2.imwrite(out_path, frame)

        writer.writerow([i, best_frame, out_path])
        print(f"Contact frame saved → {out_path}")
